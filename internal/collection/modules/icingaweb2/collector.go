...
import (
	collection2 "github.com/NETWAYS/support-collector/internal/collection"
...
var journalctlLogs = map[string]collection2.JournalElement{
...
func Collect(c *collection2.Collection) {
	if !Detect() {
	...
	if _, ok := collection2.IsGitRepository("/usr/share/icingaweb2"); ok {
	...
	if services, err := collection2.FindServices("*php*-fpm"); err == nil && len(services) > 0 {
	...
	if c.Detailed {
		...
		for name, element := range journalctlLogs {
			if service, err := collection2.FindServices(element.Service); err == nil && len(service) > 0 {
...
func CollectModuleInfo(c *collection2.Collection) {
	if !collection2.DetectGitInstalled() {
	...
	for _, file := range modulesFiles {
		...
		if _, ok := collection2.IsGitRepository(path); ok {
